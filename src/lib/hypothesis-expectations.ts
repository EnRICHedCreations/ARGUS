...
export function deriveExpectations(hypotheses:ArgusHypothesis[]):HypothesisExpectation[]{
  ...
  for(const h of hypotheses.filter(x=>x.status==="active")){
    ...
    const types=h.hypothesisType==="shared_external_driver"
      ? ["pattern_persistence","cross_source_correlation"] as const
      : h.hypothesisType==="structural_transition"
        ? ["pattern_persistence","structural_persistence"] as const
        : ["pattern_persistence"] as const;
    for(const type of types){
      const positive=h.hypothesisType!=="independent_convergence";
      out.push({
        id:eid(h.id,type), hypothesisId:h.id, expectationType:type,
        statement:type==="cross_source_correlation"?"Cross-source correlation evidence should emerge or persist before the deadline.":type==="structural_persistence"?"Structural-change signals should persist into a subsequent observation cycle.":"The originating pattern scope should recur in a subsequent observation cycle.",
        createdAt, resolvesAt:plusHours(createdAt,2), status:"open", expectedPolarity:positive?"support":"contradict", weight:type==="pattern_persistence"?0.1:0.08,
        details:{hypothesisType:h.hypothesisType,competitionGroup:h.competitionGroup}
      });
    }
  }
  return out;
...
export function resolveExpectations(expectations:HypothesisExpectation[],patterns:ArgusPattern[],signals:Signal[],nowIso:string):HypothesisExpectationOutcome[]{
  ...
  for(const e of expectations.filter(x=>x.status==="open"&&Date.parse(x.resolvesAt)<=now)){
    const group=String(e.details.competitionGroup??""); const originId=group.startsWith("pattern:")?group.slice(8):"";
    const origin=patterns.find(p=>p.id===originId); const later=origin?patterns.filter(p=>p.patternType===origin.patternType&&p.scope===origin.scope&&Date.parse(p.detectedAt)>Date.parse(e.createdAt)):[];
    ...
    if(e.expectationType==="cross_source_correlation"){
      const rows=signals.filter(s=>s.kind==="cross_source_correlation"&&Date.parse(s.observedAt)>Date.parse(e.createdAt)); matched=rows.length>0;evidenceIds=rows.map(s=>s.id).slice(0,10);
    }
    if(e.expectationType==="structural_persistence"){
      const rows=signals.filter(s=>["centrality_shift","co_occurrence","change_point"].includes(s.kind)&&Date.parse(s.observedAt)>Date.parse(e.createdAt)); matched=rows.length>=2;evidenceIds=rows.map(s=>s.id).slice(0,10);
    }
    const supports=e.expectedPolarity==="support"?matched:!matched;
    const delta=supports?e.weight:-e.weight;

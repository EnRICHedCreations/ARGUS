...
function config() {
  ...
  if (!url || !key) {
    throw new Error("ARGUS persistent memory requires SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY");
  }
  return { url, key };
}
...
async function request(path: string, init?: RequestInit) {
  ...
  if (!response.ok) {
    ...
    throw new Error(`ARGUS memory request failed (${response.status}): ${detail.slice(0, 500)}`);
  }

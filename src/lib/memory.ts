...
export async function rememberEntityGraph(entries: Array<{ observation: Observation; entities: EntityCandidate[]; relationships: TemporalRelationship[] }>) {
  ...
  for (const { observation, entities, relationships } of entries) {
    for (const entity of entities) {
      ...
      aliasRows.set(`${entity.id}:${normalizedAlias}`, {
        entity_id: entity.id,
      ...
      observationEntityRows.set(`${observation.id}:${entity.id}:${entity.mentionText}`, {
        ...
        extraction_method: "deterministic_v2",
...
export async function getGraphStats() {
  ...
  return {
    ...
    entityEventRelationships: relationships.filter(r => r.object_event_id != null).length,
    entityEntityRelationships: relationships.filter(r => r.object_entity_id != null).length,
  };
}

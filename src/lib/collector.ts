...
async function collectNwsAlerts(source:Source):Promise<Observation[]>{
 ...
 return (payload.features||[]).map((feature,index)=>{const p=feature.properties||{};const title=String(p.headline||p.event||"NWS active alert").trim();const externalId=String(feature.id||p.id||`${p.sent||observedAt}:${index}`);const publishedAt=iso(p.sent)||iso(p.effective)||observedAt;const url=String(p["@id"]||feature.id||source.url);
  // Put compact machine-readable temporal fields before free text so the 1000-char cap never drops them.
  const structured=[p.event&&`event=${p.event}`,p.severity&&`severity=${p.severity}`,p.urgency&&`urgency=${p.urgency}`,p.certainty&&`certainty=${p.certainty}`,iso(p.effective)&&`effective=${iso(p.effective)}`,iso(p.onset)&&`onset=${iso(p.onset)}`,iso(p.expires)&&`expires=${iso(p.expires)}`,iso(p.ends)&&`ends=${iso(p.ends)}`,p.areaDesc&&`area=${p.areaDesc}`,p.senderName&&`sender=${p.senderName}`,p.description&&`description=${String(p.description).replace(/\s+/g," ")}`].filter(Boolean).join(" | ").slice(0,1000);
  const {fingerprint,id}=observationId(source.id,externalId,title);return{id,sourceId:source.id,externalId,title,url,publishedAt,observedAt,summary:structured,fingerprint};});
}

export async function collect(source:Source):Promise<Observation[]>{if(!source.enabled)return[];if(source.kind==="nws_alerts_json")return collectNwsAlerts(source);const feed=await parser.parseURL(source.url);const observedAt=new Date().toISOString();return feed.items.map((item,index)=>{const title=item.title?.trim()||"Untitled observation";const url=item.link||source.url;const publishedAt=new Date(item.isoDate||item.pubDate||observedAt).toISOString();const externalId=item.guid||item.id||url||`${publishedAt}:${index}`;const summary=String(item.contentSnippet||item.content||"").replace(/\s+/g," ").trim().slice(0,1000);const{fingerprint,id}=observationId(source.id,externalId,title);return{id,sourceId:source.id,externalId,title,url,publishedAt,observedAt,summary,fingerprint};});}
